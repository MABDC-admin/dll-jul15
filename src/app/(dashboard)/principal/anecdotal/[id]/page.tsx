...
import { ChevronLeft, Calendar, User, Clock, AlertCircle } from 'lucide-react';
import { StatusBadge } from '@/components/ui/StatusBadge';
import ResolveAnecdotalForm from './ResolveAnecdotalForm';

export default async function PrincipalAnecdotalDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full space-y-6 animate-fadeIn">
      <div className="flex items-center space-x-3">
        <Link href="/principal/anecdotal" className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-50 transition text-slate-500 hover:text-slate-700 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center text-xs font-bold text-slate-500 mt-0.5 space-x-3">
                ...
                <span className="flex items-center gap-1"><User className="w-3 h-3" /> Reported by {record.teacherProfile.user.name}</span>
              </div>
            </div>
          </div>
      ...
      {record.status === 'Pending Review' ? (
        <ResolveAnecdotalForm recordId={record.id} />
      ) : (
