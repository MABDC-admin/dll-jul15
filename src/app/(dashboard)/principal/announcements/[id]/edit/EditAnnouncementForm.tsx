...
export default function EditAnnouncementForm({ announcement }: { announcement: Announcement }) {
  ...
  const [preview, setPreview] = useState<string | null>(announcement.mediaUrl);
  ...
  async function handleSubmit(formData: FormData) {
    setIsPending(true);
    try {
      let mediaUrl = preview || ''; // If they didn't remove the existing preview, keep it. If they removed it, it's empty.
      let mediaType = announcement.mediaType || ''; // Keep existing type unless changed

      // If they selected a NEW file
      if (file) {
  ...
  return (
    <form action={handleSubmit} className="space-y-6 text-sm">
      <div className="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-200">
        <div>
          <h4 className="font-bold text-slate-800 text-xs">Broadcast Status</h4>
          <p className="text-[10px] text-slate-500">Toggle whether this broadcast is actively shown to teachers.</p>
        </div>
      ...
      <div>
        ...
        {!preview ? (
          ...
          <div className="relative inline-block border border-slate-200 rounded-xl overflow-hidden bg-slate-100 max-w-md w-full">
            ...
            {(!file && announcement.mediaType === 'video') || file?.type.startsWith('video/') ? (
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-2 flex items-center gap-2 text-white text-xs font-bold">
              {(!file && announcement.mediaType === 'video') || file?.type.startsWith('video/') ? <Video className="w-3.5 h-3.5" /> : <ImageIcon className="w-3.5 h-3.5" />}
              <span className="truncate">{file ? file.name : 'Existing Media'}</span>
            </div>
          </div>
        )}
      ...
      <div>
        ...
        <input 
          name="publishAt" 
          type="datetime-local" 
          defaultValue={formatDatetimeLocal(new Date(announcement.publishAt))}
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center justify-end">
        <button disabled={isPending} type="submit" className="w-full sm:w-auto px-8 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-black rounded-xl shadow-lg hover:shadow-indigo-500/30 transition flex justify-center items-center gap-2">
          {uploadingMedia ? 'Uploading Media...' : isPending ? 'Saving...' : 'Save Changes'}
        </button>
      </div>
    </form>
  );
