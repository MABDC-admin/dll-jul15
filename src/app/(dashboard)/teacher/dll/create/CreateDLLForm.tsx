...
export default function CreateDLLForm({ 
  ...
  // Filter out subjects that already have a DLL submitted for the selected term and week
  const availableSubjects = assignedSubjects.filter(sub => {
    const hasLog = existingLogs.some(log => 
      log.learningArea === sub && 
  ...
  return (
    <div className="space-y-6">
      {availableSubjects.length > 0 && !initialSubject && (
      ...
      {availableSubjects.length === 0 && !initialSubject && assignedSubjects.length > 0 && (
          ...
          {initialSubject ? (
            ...
            >
              ...
              {availableSubjects.length === 0 && (
                <option value="" disabled>All subjects have DLLs for this week!</option>
              )}
            </select>
          ) : (
            <input 
      ...
      <button disabled={isPending || (availableSubjects.length === 0 && !initialSubject)} type="submit" className="w-full md:w-auto px-6 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold py-2.5 rounded-lg shadow-md transition">
